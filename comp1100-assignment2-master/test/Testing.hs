...
-- | Assert two 'TurtleCommand's are the same. This is a bit more
-- complex than 'assertEqual' to accound for floating-point.
assertTurtleCommandEqual :: TurtleCommand -> TurtleCommand -> TestResult
assertTurtleCommandEqual actual expected = case (actual, expected) of
  (Forward, Forward) -> OK
  (Accelerate v1, Accelerate v2) -> assertApproxEqual v1 v2
  (Turn t1, Turn t2) -> assertApproxEqual t1 t2
  (PenUp, PenUp) -> OK
  (PenDown, PenDown) -> OK
  _ -> Fail (show actual ++ " is not the same command as\n" ++ show expected)

-- | Assert two lists of 'TurtleCommand's are the same. This is an
-- example of 'testList', below.
assertTurtleCommandsEqual :: [TurtleCommand] -> [TurtleCommand] -> TestResult
assertTurtleCommandsEqual actual expected = testList
  ( assertEqual (length actual) (length expected)
  : zipWith assertTurtleCommandEqual actual expected
  )
